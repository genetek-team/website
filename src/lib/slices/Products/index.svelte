<script lang="ts">
	...
	/** @type {import("@prismicio/client").Content.ProductDocument[]} */
	export let products = [];
</script>
		...
		{#each products as product, index}
			...
			>
				<div class="col-span-1 flex flex-col justify-center gap-4">
					<h3 class="text-4xl">
						<PrismicText field={product.data.name} />
					</h3>
					<div class="max-w-md">
						<PrismicRichText field={product.data.description} />
					</div>

					<PrismicLink document={product} class="z-10 after:absolute after:inset-0 hover:underline">
						<PrismicText field={product.data.name} />
					</PrismicLink>
				</div>
