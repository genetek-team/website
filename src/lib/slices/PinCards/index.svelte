<script lang="ts">
	...
	export let slice: Content.PinCardsSlice;
	...
	onMount(() => {
		...
		const isMobile = window.innerWidth < 300; // Use your desired breakpoint for mobile
		const offset = isMobile ? 50 : 100; // Adjust the offset based on mobile
		...
		gsap.fromTo(
			...
			},
			{
				...
				duration: 0.8,
				scrollTrigger: {
					trigger: '.pincards__heading',
					start: `top bottom+=${offset}%`, // Use the adjusted offset for mobile
...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	<div
		class="pincards__glow absolute -z-10 aspect-video w-full max-w-2xl rounded-full bg-blue-700/50 mix-blend-screen blur-[120px] filter"
	/>

	<h2 class="pincards__heading">
		<PrismicRichText
